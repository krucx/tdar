	...
	for i in range(1,462):
		...
		with open(file,'r') as f:
			for row in f:
				...
				labels[int(center_y)//32][int(center_x)//32] = l
...
def show_random(X,Y):
	...
	img = cv2.cvtColor(X[idx],cv2.COLOR_RGB2BGR)
	for i in range(Y.shape[1]):
		for j in range(Y.shape[2]):
			if Y[idx][i][j][0]==1:
				x1 = Y[idx][i][j][1] - Y[idx][i][j][3]/2
				x2 = Y[idx][i][j][1] + Y[idx][i][j][3]/2
				y1 = Y[idx][i][j][2] - Y[idx][i][j][4]/2
				y2 = Y[idx][i][j][2] + Y[idx][i][j][4]/2
